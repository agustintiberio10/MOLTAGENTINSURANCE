  ...
  for (const pool of proposedPools) {
    ...
    try {
      ...
      if (parseFloat(balance) < premiumUsdc) {
        console.log(`[MoltX-Retry] Insufficient USDC for "${pool.description}". Need ${premiumUsdc}, have ${balance}.`);
        continue;
      }
      ...
      if (pool.moltxPostId) {
        try {
          const updateReply =
            ...
            `Contract: ${state.contractAddress}\n\n` +
            `Join: approve USDC → joinPool(${pool.onchainId}, amount)\n` +
...
async function runMoltxHeartbeat() {
  ...
  console.log(`[MoltX-Stats] Replies today: ${getMoltxDailyReplies(state)}/${MAX_DAILY_REPLIES} | Posts today: ${getMoltxDailyPosts(state)}/${MAX_DAILY_POSTS}`);
  ...
  // (c.5) Retry proposed pools
  if (blockchain) {
    await retryProposedPoolsMoltx(blockchain, moltx, state);
  }
